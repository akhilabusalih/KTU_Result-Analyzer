  ...
  return (
    <div className="grid grid-cols-5 gap-4">
      {kpiData.map((kpi, index) => (
        <div key={index} className={`rounded-lg p-4 shadow-sm hover:shadow-md transition-all ${
          isDarkMode ? 'bg-gray-800' : 'bg-white'
        }`}>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <p className={`text-xs mb-1.5 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{kpi.title}</p>
              <p className={`text-2xl ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{kpi.value}</p>
            </div>
            <div className={`${kpi.color} ${isDarkMode ? kpi.darkColor : ''} ${kpi.iconColor} ${isDarkMode ? kpi.darkIconColor : ''} p-2 rounded-lg`}>
