  ...
  return (
    <div className={`rounded-lg p-4 shadow-sm transition-colors ${
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className={`text-base ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>AI Insights</h3>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {insights.map((insight, index) => (
          <div 
            key={index} 
            className={`${insight.color} ${isDarkMode ? insight.darkColor : ''} border ${insight.borderColor} ${isDarkMode ? insight.darkBorderColor : ''} rounded-lg p-3`}
          >
            <div className="flex items-start gap-2">
              <div className={`${insight.iconColor} ${isDarkMode ? insight.darkIconColor : ''} mt-0.5`}>
                <insight.icon className="w-4 h-4" />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-xs mb-0.5 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{insight.title}</p>
                <p className={`text-sm mb-0.5 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{insight.value}</p>
