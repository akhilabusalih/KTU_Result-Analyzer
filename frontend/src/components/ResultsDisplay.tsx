import { FileText, TrendingUp, Users, Award, FileDown, FileSpreadsheet, UserX, Trophy, RefreshCw, RotateCcw, ArrowUp, CheckCircle2 } from "lucide-react";
...
export function ResultsDisplay({
  ...
  const handleReprocessingComplete = useCallback(() => {
    setIsReprocessing(false);
    if (pendingDepartment && onDepartmentChange) {
      onDepartmentChange(pendingDepartment);
    }
    setPendingDepartment(null);
  }, [pendingDepartment, onDepartmentChange]);
  ...
  // Reverse lookup: full name → key
  const departmentKeyFromName: Record<string, string> = Object.fromEntries(
    Object.entries(departmentNames).map(([k, v]) => [v, k])
  );

  // Normalise: accept both short keys ("computer") and full names ("Computer Science Engineering")
  const departmentKey = departmentNames[department]
    ? department
    : departmentKeyFromName[department] ?? "computer";
  const departmentDisplayName = departmentNames[departmentKey] ?? department;
  ...
  const getGradeColor = (grade: string) => {
    switch (grade) {
      case "S": return "bg-emerald-500 text-white shadow-sm";
      case "A+": return "bg-green-500 text-white shadow-sm";
      case "A": return "bg-green-600 text-white shadow-sm";
      case "B+": return "bg-blue-400 text-white shadow-sm";
      case "B": return "bg-blue-500 text-white shadow-sm";
      case "C+": return "bg-amber-400 text-white shadow-sm";
      case "C": return "bg-amber-500 text-white shadow-sm";
      case "D": return "bg-orange-500 text-white shadow-sm";
      case "F": return "bg-red-600 text-white shadow-sm";
      case "P": return "bg-red-500 text-white shadow-sm";
      case "Absent": return "bg-gray-400 text-white shadow-sm";
  ...
  const exportToPDF = () => {
    ...
    doc.text(`Department: ${departmentDisplayName}`, 10, 30);
  ...
  const subjects = ["EST200", "MCN201", "MAT203", "CST201", "CST203", "CST205", "CSL201", "CSL203"];
  const gradeTypes = ["S", "A+", "A", "B+", "B", "C+", "C", "D", "P", "F"];

  const getGradeCount = (subject: string, grade: string) => {
    return studentResults.filter(s => (s as any)[subject] === grade).length;
  };

  const getPassCount = (subject: string) => {
    const failGrades = ["F", "Absent"];
    return studentResults.filter(s => !failGrades.includes((s as any)[subject])).length;
  };

  const getFailCount = (subject: string) => {
    return studentResults.filter(s => (s as any)[subject] === "F").length;
  };

  const getPassPercentage = (subject: string) => {
    return ((getPassCount(subject) / studentResults.length) * 100).toFixed(2);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className="flex items-start justify-between gap-6">
          <div className="flex items-start gap-4 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="font-bold bg-gradient-to-r from-gray-900 to-gray-700 dark:from-gray-100 dark:to-gray-300 bg-clip-text text-transparent mb-2 text-[20px]">Analysed Result</h2>
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="text-right border-l border-gray-200 dark:border-gray-700 pl-6">
              ...
              <p className="text-lg font-semibold bg-gradient-to-r from-blue-600 to-teal-600 dark:from-blue-400 dark:to-teal-400 bg-clip-text text-transparent whitespace-nowrap">{departmentDisplayName}</p>
              {onDepartmentChange && (
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="electrical">Electrical and Electronics Engineering</SelectItem>
                      </SelectContent>
                    </Select>
                    {!detailedAnalysis && (
                      <div 
                    ...
                    <div className="absolute top-full left-0 right-0 mt-3 z-50 p-3.5 bg-amber-50 dark:bg-amber-900/30 border-2 border-amber-400 dark:border-amber-600 rounded-lg shadow-xl animate-fade-in">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-semibold text-amber-800 dark:text-amber-300 mb-1">Feature Disabled</p>
        ...
        <Card className="p-4 bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 transition-colors overflow-x-auto shadow-lg">
            ...
            <table className="min-w-full border-collapse">
              <thead>
                <tr className="bg-gradient-to-r from-blue-600 to-teal-600 dark:from-blue-500 dark:to-teal-500">
                  ...
                  {subjects.map(sub => (
                    <th key={sub} className="px-3 py-3 text-center text-sm font-bold text-white border-r border-white/20">{sub}</th>
                  ))}
              ...
              <tbody className="bg-white dark:bg-gray-900">
                {studentResults.map((student, index) => (
                  ...
                  >
                    ...
                    {subjects.map(sub => (
                      <td key={sub} className="px-3 py-3 text-center border-b border-gray-200 dark:border-gray-700">
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${getGradeColor((student as any)[sub])}`}>
                          {(student as any)[sub]}
                        </span>
                      </td>
                    ))}
          ...
          <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              {[
                { grade: "S", label: "Outstanding", color: "bg-emerald-500" },
                { grade: "A", label: "Excellent", color: "bg-green-500" },
                { grade: "B", label: "Good", color: "bg-blue-500" },
                { grade: "C", label: "Average", color: "bg-amber-500" },
                { grade: "D", label: "Below Avg", color: "bg-orange-500" },
                { grade: "F", label: "Fail", color: "bg-red-600" },
                { grade: "Ab", label: "Absent", color: "bg-gray-400" },
              ].map(item => (
                <div key={item.grade} className="flex items-center gap-1.5">
                  <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${item.color} text-white shadow-sm`}>{item.grade}</span>
            ...
            <table className="min-w-full border-collapse">
              <thead>
                <tr className="bg-gradient-to-r from-blue-600 to-teal-600 dark:from-blue-500 dark:to-teal-500">
                  ...
                  {subjects.map(sub => (
                    <th key={sub} className="px-3 py-2.5 text-center text-sm font-bold text-white border-r border-white/20">{sub}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="bg-white dark:bg-gray-900">
                <tr className="bg-emerald-50 dark:bg-emerald-950 hover:bg-emerald-100 dark:hover:bg-emerald-900 transition-all duration-200">
                  <td className="px-4 py-2.5 text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">Pass Percentage</td>
                  {subjects.map(sub => (
                    <td key={sub} className="px-3 py-2.5 text-center text-sm font-semibold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{getPassPercentage(sub)}%</td>
                  ))}
                </tr>
                <tr className="bg-gray-50 dark:bg-gray-900 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all duration-200">
                  ...
                  {subjects.map(sub => (
                    <td key={sub} className="px-3 py-2.5 text-center text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{getPassCount(sub)}</td>
                  ))}
                </tr>
                <tr className="bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all duration-200">
                  ...
                  {subjects.map(sub => (
                    <td key={sub} className="px-3 py-2.5 text-center text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{getFailCount(sub)}</td>
                  ))}
                </tr>
                {gradeTypes.map((grade, gi) => (
                  <tr key={grade} className={`${gi % 2 === 0 ? "bg-gray-50 dark:bg-gray-900" : "bg-white dark:bg-gray-800"} hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all duration-200`}>
                    <td className="px-6 py-3 text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">No of {grade} grades</td>
                    {subjects.map(sub => (
                      <td key={sub} className="px-4 py-3 text-center text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{getGradeCount(sub, grade)}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
            ...
            <table className="min-w-full border-collapse">
              <tbody className="bg-white dark:bg-gray-900">
                <tr className="bg-blue-50 dark:bg-blue-950 hover:bg-blue-100 dark:hover:bg-blue-900 transition-all duration-200">
                  <td className="px-6 py-3 text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">Total Students (REGULAR)</td>
                  <td className="px-6 py-3 text-center text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{studentResults.length}</td>
                </tr>
                <tr className="bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all duration-200">
                  <td className="px-6 py-3 text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">Total Failed (REGULAR) (students having one or more arrears)</td>
                  <td className="px-6 py-3 text-center text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">
                    {studentResults.filter(s => parseInt(s.noArrea) > 0).length}
                  </td>
                </tr>
                <tr className="bg-gray-50 dark:bg-gray-900 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all duration-200">
                  <td className="px-6 py-3 text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">Total Passed (REGULAR) (students having no arrears)</td>
                  <td className="px-6 py-3 text-center text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">
                    {studentResults.filter(s => s.noArrea === "0").length}
                  </td>
                </tr>
                <tr className="bg-emerald-50 dark:bg-emerald-950 hover:bg-emerald-100 dark:hover:bg-emerald-900 transition-all duration-200">
                  <td className="px-6 py-3 text-sm font-bold text-gray-900 dark:text-gray-100">Pass Percentage (REGULAR)</td>
                  <td className="px-6 py-3 text-center text-lg font-bold text-emerald-600 dark:text-emerald-400">
                    {((studentResults.filter(s => s.noArrea === "0").length / studentResults.length) * 100).toFixed(2)}%
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <Card className="p-4 bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 transition-colors overflow-x-auto shadow-lg">
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
            <table className="min-w-full border-collapse">
              <thead>
                <tr className="bg-gradient-to-r from-blue-600 to-teal-600 dark:from-blue-500 dark:to-teal-500">
                  {["SubCode", "Pass %", "Pass", "Fail", "S", "A+", "A", "B+", "B", "C+", "C", "D", "P", "F", "FE/I"].map(h => (
                    <th key={h} className="px-3 py-2.5 text-center text-sm font-bold text-white border-r border-white/20 last:border-r-0">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="bg-white dark:bg-gray-900">
                {subjects.map((sub, si) => (
                  <tr key={sub} className={`${si % 2 === 0 ? "bg-gray-50 dark:bg-gray-900" : "bg-white dark:bg-gray-800"} hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all duration-200`}>
                    <td className="px-3 py-2.5 text-center text-sm font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{sub}</td>
                    <td className="px-3 py-2.5 text-center text-sm font-semibold text-emerald-600 dark:text-emerald-400 border-b border-gray-200 dark:border-gray-700">{getPassPercentage(sub)}</td>
                    <td className="px-3 py-2.5 text-center text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{getPassCount(sub)}</td>
                    <td className="px-3 py-2.5 text-center text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{getFailCount(sub)}</td>
                    {["S", "A+", "A", "B+", "B", "C+", "C", "D", "P", "F"].map(g => (
                      <td key={g} className="px-3 py-2.5 text-center text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">{getGradeCount(sub, g)}</td>
                    ))}
      ...
      <AnimatePresence>
        {isReprocessing && (
          <ReprocessingModal onComplete={handleReprocessingComplete} />
        )}
      </AnimatePresence>
    </div>
  );
...
// Reprocessing Modal Component (department change)
function ReprocessingModal({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 border border-gray-200 dark:border-gray-700"
      >
        <div className="flex flex-col items-center">
          ...
          <div className="relative w-40 h-40 mb-6">
            <svg className="w-40 h-40 -rotate-90" viewBox="0 0 140 140">
              ...
              <defs>
                <linearGradient id="reprocess-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
